...
class Labarugi extends CI_Controller {
	...
	public function index()
	{
		$page=$this->input->get('per_page');
		  ...
		  $config['base_url'] = base_url().'index.php/Admin/Laporan/Labarugi?';   //url yang muncul ketika tombol pada paging diklik
		  $config['total_rows'] = $this->Mlabarugi->count_labarugi(); // jlh total labarugi
		  ...
		  $data['title'] = 'Laporan Laba Rugi'; //judul title
		  $data['box_title'] = 'Daftar Laporan'; //judul title
		  $data['total'] = $this->Mlabarugi->count_labarugi(); // jlh total labarugi;
		  $data['judule'] = 'labarugi'; //judul title

		  $data['qlabarugi'] = $this->Mlabarugi->get_alllabarugi($batas,$offset); //query model semua labarugi


		  $content="admin/labarugi";
		  $this->load->view('template/header');
		  $this->load->view('template/topbar');
		  $this->load->view('template/leftbar');
		  $this->load->view('admin/laporan/labarugi',$data);
		  $this->load->view('template/footer');

}

	function _insert($data){
		$this->Mlabarugi->_insert($data);
	}

	function _update($data,$id){
		$this->Mlabarugi->_update($data,$id);
	}

	function _get_where($kolom,$id){
		$query = $this->Mlabarugi->_get_where($kolom,$id);
